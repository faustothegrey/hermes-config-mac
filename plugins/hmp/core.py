...
class HMPStatusStore(object):
    ...
    def queue(self, message_id: str, body: Dict[str, Any], from_peer: str, to_peer: str, text: str, chat_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, NULL, NULL, ?, ?, ?, NULL, NULL, NULL)
                """,
                (message_id, idempotency_key, from_peer, to_peer, chat_id, text, "queued", raw_json, ts, ts),
            )
        # Return the accepted queue state even if the consumer claims the row
        # immediately after commit; /hmp/send must acknowledge enqueue, not the
        # consumer's transient processing state.
        return {
    ...
    def dequeue(self) -> Optional[Dict[str, Any]]:
        """Claim the oldest queued HMP message and mark it delivering."""
        ts = now_ts()
        with self._connect() as conn:
            conn.execute("BEGIN IMMEDIATE")
            row = conn.execute(
                "SELECT * FROM hmp_gateway_messages WHERE status = ? ORDER BY accepted_at LIMIT 1",
                ("queued",),
            ).fetchone()
            if row is None:
                conn.commit()
            ...
            conn.execute(
                "UPDATE hmp_gateway_messages SET status = ?, updated_at = ? WHERE message_id = ?",
                ("delivering", ts, message_id),
            )
            conn.commit()
