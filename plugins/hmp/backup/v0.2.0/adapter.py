...
class HMPAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig) -> None:
        ...
        self.sse = SSEStreamStore()
        self._current_message: Dict[str, str] = {}  # chat_id → latest message_id in processing
        self._runner = None
        self._site = None
        ...
        self._app.add_routes(
            [
                ...
                web.get(r"/hmp/stream/{message_id}", self.hmp_stream),
            ]
        )
    ...
    async def send(
        ...
        if original_id:
            mid = str(original_id)
            # Push progress to SSE subscribers in real time
            await self.sse.push_progress(mid, str(content))
            # Also track this as current message for this chat
            self._current_message[str(chat_id)] = mid
            # Complete the store (backward compat with poll/send_and_wait)
            self.store.complete(
                mid,
                str(content),
            ...
            # Signal SSE subscribers that this is a completed event
            await self.sse.push_complete(mid, str(content))
    ...
    async def send_or_update_status(
        ...
        """Tool progress / status updates from the gateway.

        Push each update as an SSE progress event so the client sees
        tool-level progress in real time.
        """
        message_id = self._current_message.get(str(chat_id))
        if message_id:
            await self.sse.push_progress(message_id, str(content))
        return SendResult(success=True, message_id="status_" + uuid.uuid4().hex[:12])
        ...
        return web.json_response(
            {
                ...
                "endpoints": [
                    ...
                    "/hmp/send_and_wait",
                    "/hmp/poll/{message_id}",
                    "/hmp/stream/{message_id}",
                ],
                "features": ["sse_streaming"],
            }
        )
    ...
    async def hmp_stream(self, request: web.Request) -> web.Response:
        """Server-Sent Events endpoint.  Streams progress/complete events."""
        message_id = request.match_info["message_id"]

        # If the message is already completed, send the final response immediately
        item = self.store.get(message_id)
        if item and item.get("status") == "completed":
            resp = _sse_format("complete", item.get("response_text", "") or "")
            return web.Response(text=resp, content_type="text/event-stream")
        ...
        try:
            async for event in self.sse.subscribe(message_id):
                line = _sse_format(event.event_type, event.data)
                try:
            ...
            await self.sse.cleanup(message_id)
    ...
    async def _accept_hmp_message(self, request: web.Request, body: Dict[str, Any]):
        ...
        self._current_message[str(from_peer)] = message_id
        ...
        try:
            ...
            await self.sse.push_error(message_id, str(exc))
            return {"accepted": False, "message_id": message_id, "status": "failed", "error": str(exc)}, 500

        # handle_message() spawns a background task and returns quickly.
        # The agent's response comes asynchronously via send() — that's where
        # store.complete() and SSE streaming happen.
