...
5. **Voice/TTS defaults are split between live command state and persisted config.** `/voice tts` toggles in-memory `_voice_tts`; to make TTS the default for future `/voice on`, persist `voice.auto_tts: true`. Provider voice is separate under `tts.<provider>.voice`; for Edge Italian output use `tts.provider: edge` and `tts.edge.voice: it-IT-ElsaNeural`. `voice.enabled` / `voice.continuous` may exist in config but older CLI code may not auto-start voice mode from them, so verify by restarting Hermes and running `/voice status` rather than assuming startup reads those keys.
