...
6. CLAUDE ok:false WITH ZEROS = usually an AUTH-MODE mismatch, not "logged out". The /usage subscription bars ("Current session X% used", "Current week (all models)") ONLY exist in **subscription mode** (header shows "Claude Pro/Max"). If claude is authed via an OAuth token (`CLAUDE_CODE_OAUTH_TOKEN` env, `sk-ant-oat...`) the header shows "Claude API" and the ONLY /usage content is session cost ($0) + stats — no quota bars → parser returns zeros. FIX: authenticate the service as claude.ai subscription (interactive `claude /logout` then `claude /login` → choose "Claude account with subscription", NOT Console API key) and REMOVE the token: `authMethod:"claude.ai"` = Pro (bars present), `authMethod:"oauth_token"` = API (no bars). The token env var OVERRIDES the Keychain login, so on macOS check BOTH the launchd plist EnvironmentVariables AND ~/.zshrc for a `CLAUDE_CODE_OAUTH_TOKEN` export and comment/remove it. Verify quickly: `env -u CLAUDE_CODE_OAUTH_TOKEN claude auth status` (no token) vs with — compare authMethod. launchd does NOT read ~/.zshrc, so for the SERVICE only the plist matters; the .zshrc export only affects your interactive terminals.
7. TRUST DIALOG ORDER FLIPPED (claude >=2.1.250): "No, exit" is now the FIRST/default row, so a bare Enter QUITS claude (tmux session dies, capture shows "no server running"). FIX (belt-and-suspenders): (a) set `hasTrustDialogAccepted:true` for the workdir in ~/.claude.json to skip the dialog entirely — also set hasClaudeMdExternalIncludesApproved/WarningShown true; (b) in the scraper send **Down then Enter** (not bare Enter) so it still lands on "Yes, I trust" if the dialog reappears. Skill pitfall #2 (never send bare Escape) still holds.
8. /usage is a TABBED dialog (Settings·Status·Config·Usage·Stats) that opens on the SESSION cost tab; the quota bars are rendered inline on that same first screen in subscription mode (no tab navigation needed) — capture -S -220 catches them. Right-arrow just cycles Stats/Status/Config, don't rely on it.
