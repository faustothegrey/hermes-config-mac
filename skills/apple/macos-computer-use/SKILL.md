...
Before spending any effort, verify the machine can run cua-driver:

```bash
uname -m
```

- If it returns `arm64` -> Apple Silicon. Proceed with install.
- If it returns `x86_64` -> Intel Mac. **cua-driver does NOT ship Intel
  builds** (see [trycua/cua#1493](https://github.com/trycua/cua/issues/1493)).
  The `computer_use` toolset is unavailable. Fall back to `browser_*` tools
  for web automation. Report the blocker honestly.

## Failure modes

- **"cua-driver not installed"** -> Run `hermes computer-use install`.
  On first install, this fetches the upstream script. Requires
  macOS + the permissions listed below.
- **Intel Mac - no binary available** -> If `uname -m` is `x86_64`, the
  install step will report "Latest CUA release has no Intel asset". This
  is a known upstream limitation. Cannot be worked around.
- **Permission prompts on first use** -> macOS will ask for Accessibility
  and Screen Recording access the first time cua-driver runs. Pre-grant
  them via: System Settings > Privacy & Security > Screen Recording /
  Accessibility. Add the cua-driver binary (Cmd+Shift+G ->
  /usr/local/bin/cua-driver) and toggle ON.
