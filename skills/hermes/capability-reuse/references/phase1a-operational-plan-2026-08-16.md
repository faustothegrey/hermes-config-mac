...
**Supersedes:** nothing (Phase 0 is closed; this opens Phase 1a)
...
| Item | Value |
|---|---|
| Phase 0 status | CLOSED (cohort peer141↔peer70, holdout 3/3, 100% ≥ 85%) |
| Runtime mode | shadow on peer141 + peer70 (post-Phase-0 baseline) |
| Active-mode drop-ins | removed from activation path; preserved as artifacts in `evidence/phase0-closed-2026-08-16/dropins/` |
| Registry | 3 capabilities; only `hmp-healthcheck@1.0.0` trusted |
| Retriever | text-matching (token+bigram), no embeddings |
| Current thresholds | intervention 0.65, min margin 0.05 (calibrated on n=3) |
