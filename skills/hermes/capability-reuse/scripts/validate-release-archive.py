...
def verify_report(zf, report_path, label):
    ...
    # P0-7 (reviewer 2026-08-16): a report with status=FAIL but no "failed"
    # field must NOT pass. Check both the numeric failures field AND status.
    if data.get("failed", 0) not in (0, None):
        raise AssertionError("%s report has failures" % label)
    status = str(data.get("status") or "").strip().upper()
    verdict = str(data.get("verdict") or "").strip().upper()
    if status and status != "PASS":
        raise AssertionError("%s report declares status=%s (must be PASS)" % (label, data.get("status")))
    if verdict and verdict != "PASS":
        raise AssertionError("%s report declares verdict=%s (must be PASS)" % (label, data.get("verdict")))
...
def main(argv=None):
    ...
    with zipfile.ZipFile(args.archive) as zf:
        ...
        # P0-6 (reviewer 2026-08-16): il manifest usa `plugin_tree_hash`
        # (e anche source_plugin_tree_sha256 / plugin_hash). Tutti i campi
        # presenti devono concordare con l'hash reale calcolato.
        declared_plugin_hash = (
            manifest.get("plugin_tree_hash")
            or manifest.get("source_plugin_tree_sha256")
            or manifest.get("plugin_hash")
        )
        for key in ("plugin_tree_hash", "source_plugin_tree_sha256", "plugin_hash"):
            v = manifest.get(key)
            if v and str(v).lower() != actual_plugin_hash.lower():
                raise AssertionError(
                    "plugin hash mismatch: manifest.%s=%s actual=%s" % (key, v, actual_plugin_hash))
        ...
        conformance = verify_report(zf, validation.get("conformance_report") or manifest.get("conformance_report"), "conformance")
        # P0 round-3 (reviewer 2026-08-16): the conformance report must belong
        # to the EXACT plugin tree being shipped. verify_report() returns the
        # parsed report; if it declares artifact_identity.plugin_tree_hash, it
        # MUST equal the hash computed from this archive. Fail-closed: a
        # mismatched (or missing-when-expected) identity is a release blocker.
        _conf_identity = conformance.get("artifact_identity") if isinstance(conformance.get("artifact_identity"), dict) else {}
        _conf_tree = str(_conf_identity.get("plugin_tree_hash") or "").strip().lower()
        if _conf_tree and _conf_tree != actual_plugin_hash.lower():
            raise AssertionError(
                "conformance artifact identity mismatch: report=%s actual=%s" % (_conf_tree, actual_plugin_hash))
        if not _conf_tree:
            raise AssertionError("conformance report missing artifact_identity.plugin_tree_hash (required for release identity)")
        ...
        # P0-6 (reviewer 2026-08-16): sidecar, manifest embedded e bytes reali
        # devono concordare TUTTI quando i valori sono presenti (un placeholder
        # testuale nel manifest non e' un valore hash reale e non puo' essere
        # confrontato — vedi pattern "external sidecar required").
        import re as _re
        manifest_sha = str(manifest.get("artifact_sha256") or "").strip().lower()
        is_real_hash = bool(_re.fullmatch(r"[0-9a-f]{64}", manifest_sha))
        if expected_sha and is_real_hash and expected_sha.lower() != manifest_sha:
            raise AssertionError(
                "archive hash inconsistency: sidecar=%s manifest=%s" % (expected_sha.lower(), manifest_sha))
        if expected_sha and actual_archive_sha.lower() != expected_sha.lower():
            raise AssertionError("archive SHA256 mismatch: expected %s actual %s" % (expected_sha, actual_archive_sha))
        if is_real_hash and actual_archive_sha.lower() != manifest_sha:
            raise AssertionError("archive SHA256 mismatch: manifest %s actual %s" % (manifest_sha, actual_archive_sha))
        if not (expected_sha or is_real_hash):
