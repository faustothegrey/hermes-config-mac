...
class V2418Points5689Tests(unittest.TestCase):
    ...
    def test_rejection_reason_names_candidate_filter_not_score_when_score_passes(self):
        result = retriever.retrieve(
            session_id="p5-filter-reason",
            user_message="check HMP health for peer128",
            hook_context={"platform": "hmp", "sender_id": "peer106"},
            shadow_mode=False,
        ...
        self.assertGreaterEqual(d["top_score"], d["intervention_threshold"])
        self.assertGreaterEqual(d["score_margin"], d["minimum_margin"])
        self.assertNotEqual("below_threshold_or_margin", d["eligibility_reason"])
        self.assertIn(d["eligibility_reason"], d["candidates"][0]["ineligibility_reasons"])
        self.assertTrue(any(r.endswith(": " + d["eligibility_reason"])
                            for r in d["filter_rejection_reasons"]))
