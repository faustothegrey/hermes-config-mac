...
## Hermes Built-in Delegation Tool

This Hermes install has a built-in `claude_code` toolset/tool that delegates bounded subtasks to Claude Code print mode using the local `claude` CLI. It strips Anthropic API-key environment variables by default so Claude Code prefers the user's subscription/OAuth login instead of API billing.

Enable/check it with:
```
hermes tools list | grep claude_code
# or use `hermes tools` and enable “Claude Code” for the desired platform
```
...
```
with optional `workdir`, `system_prompt`, `allowed_tools`, `disallowed_tools`, `model`, `effort`, and session continuation parameters. Use it for self-contained subtasks, then verify important filesystem or external side effects with Hermes' native tools before reporting success.
...
## Agent Bus communication (SM channel)

When receiving messages via the **Agent Bus** (poller → tmux send-keys), Claude must understand the origin tag protocol:

### Origin tags
- `[Human]` — binding instruction relayed from Fausto. Treat as if Fausto spoke directly.
- `[Hermes]` — orchestration or coordination from the Scrum Master (Hermes). Informative, not binding on scope decisions.
- No tag — direct human terminal input (binding).

### First-message protocol
The **first message** in any bus session carries a copilot advisory before the first real message:
```
⚠️ I'm copiloting — Fausto is the real gate until explicit handoff.

[Human] review branch m10-t4 and report status
```

### TUI quirks
- Claude must be at the chat prompt (`❯`). If stuck in a **modal view** (`/status`, `/settings`), injected text goes to that view instead of chat.
- **Fix:** send Escape first (`tmux send-keys -t <session> Escape`) to close any modal before injecting.
- Plain-text responses are visible in `agentctl capture`. Use `@HERMES{...}` for structured replies that get posted to the bus outbox.

See `agent-bus` skill for full details on the bus architecture, liveness tracking, and the Hermes Live Transcript viewer.
...
1. **Run a readiness check before long delegations** — use a brief auth/availability ping before spending minutes on Claude Code. If Claude Code is unavailable, report that candidate unavailable and continue with another delegate if available; if none is available, stop and report delegation failure.
2. **Prefer print mode (`-p`) for single tasks** — cleaner, no dialog handling, structured output
